...
import { takeActionData } from '@/data/takeActionData'
...
const TakeAction = () => {
    return (
        <div className="px-5 py-15">
            ...
            <motion.div
                className='flex flex-col gap-5 py-5 md:grid md:grid-cols-2 xl:grid-cols-3'
                initial={{ opacity: 0, y: 40 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, ease: "easeOut" }}
                viewport={{ once: true }}>
                {
                    takeActionData.map((data) => (
                        <div key={data.name}>
                            <Image src={data.photo} alt={data.name} className='h-80 md:w-full'/>
